...
class PactMixin(NsMixin):
    ...
    def _on_built(self) -> None:
        ...
        #  P-3.3 for the ESTIMATOR: centre and scale each inverter's channels on
        #  its OWN reference (N, r).  The observation keeps the pooled (r,)
        #  reference so every arm sees the same feature.  See
        #  Coupling.geometric_reference for what the pooled scale cost here.
        self._ref_i, self._scale_i = self.coupling.geometric_reference(per_agent=True)
        self.rls: List[RLS] = [self._new_rls(int(c.numel())) for c in cols]
        self._n_resets = 0
    ...
    # -- the estimator, per agent -------------------------------------------

    def _new_rls(self, dim: int) -> RLS:
        """A fresh ``pact1.core.RLS`` for one inverter, with its state held in
        float64.  ``core.py`` is not modified: its arithmetic is dtype-agnostic,
        so upcasting the tensors it allocates is enough.  The rank-one
        covariance update ``(P - K (P psi)') / mu`` cancels catastrophically in
        float32 once the regressor is ill-conditioned, which is how the
        case141 run lost positive-definiteness; the per-inverter RLS is 4x4 at
        most, so float64 is free."""
        r = RLS(1, dim, self.pact_params, batch=1)
        r.beta = r.beta.double()
        r.P = r.P.double()
        return r

    def _rls_state_ok(self, r: RLS) -> bool:
        return bool(torch.isfinite(r.beta).all()) and bool(torch.isfinite(r.P).all())

    # -- the basis, per agent ---------------------------------------------

    def _full_psi(self) -> Tensor:
        """``(N, full_dim)``: [1, (A,) centred channels] on each inverter's OWN
        reference; per-agent live columns are selected from this by
        ``_live_cols``.  float64, for the estimator."""
        psi = self.coupling.design(self._x, self._ref_i, self._scale_i).double()  # (N, 1+r)
        if self._known_driver:
            a = torch.full((self.n_ag, 1), float(self._A), dtype=torch.float64)
    ...
    def _after_disturbance(self) -> None:
        ...
        y = self._y_prev.clamp(-self.pact_params.y_clip, self.pact_params.y_clip).double()
        ...
        if self._have_prev:
            ...
            self._update_panel(ahead.float(), y.float(), self._psi_prev)
            for i in range(self.n_ag):
                self.rls[i].update(self._rows(self._psi_prev, i), y[i].reshape(1, 1))
                #  An estimator whose state has gone non-finite never recovers
                #  on its own (NaN in beta or P stays NaN), so P-7.1's "a
                #  diverging estimate can fail to help" would silently become
                #  "the method is off for the rest of training" -- measured on
                #  case141: 89,756 non-finite predictions and applied trust
                #  0.24 from episode 31 on.  Re-initialise that inverter's
                #  estimator (beta = 0, P = p0 I) and count it; trust for it
                #  then re-arms through the warm-up as at the start of the run.
                if not self._rls_state_ok(self.rls[i]):
                    self.rls[i] = self._new_rls(int(self._live_cols[i].numel()))
                    self._n_resets += 1
        ...
        for i in range(self.n_ag):
            ...
            pred[i] = float(self.rls[i].predict(row)[0, 0])
            conf[i] = float(confidence(row, self.rls[i].P, self.pact_params, int(row.shape[-1]))[0, 0])
    ...
    def _target_beta(self) -> Tensor:
        """beta* in the estimator's own coordinates, ``(N, full_dim)``.
        ...
        """
        ref, scale = self._ref_i, self._scale_i  # (N, r), the estimator's own coordinates
        if self._known_driver:
            b1 = beta_star(torch.tensor([1.0]), self.ns)[0]
            return torch.cat([torch.zeros(self.n_ag, 1), (b1 * ref).sum(-1, keepdim=True), b1 * scale], dim=-1)
        bt = beta_star(torch.tensor([self._A]), self.ns)[0]
        return torch.cat([(bt * ref).sum(-1, keepdim=True), bt * scale], dim=-1)
    ...
    def _update_panel(self, ahead: Tensor, y: Tensor, psi_prev: Tensor, decay: float = 0.99) -> None:
        ...
        if float(tgt.norm()) <= 1e-9:
            return
        for i in range(self.n_ag):
            t = tgt[i][self._live_cols[i]]
            b = self.rls[i].beta[0, 0].float()
            tn = float(t.norm())
            if tn <= 1e-12:
                continue
            bn = float(b.norm())
    ...
    def _layer_info(self) -> Dict[str, float]:
        ...
        info.update({
            ...
            "pact_resets": float(self._n_resets),
        })
        return info
